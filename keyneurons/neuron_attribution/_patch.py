...
class Patch(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        #print("self.ff: ", self.ff)
        x = self.ff(x)
        if self.mode == "replace":
            x[:, self.mask_idx, :] = self.acts
        elif self.mode == "suppress":
            for pos in self.target_positions:
                x[:, self.mask_idx, pos] = 0
                #x[:, self.mask_idx, pos] *= 0.1
        elif self.mode == "enhance":
            for pos in self.target_positions:
                x[:, self.mask_idx, pos] *= self.update_value
        else:
...
def patch_ff_layer(
    ...
    """
    transformer_layers = get_attributes(model, transformer_layers_attr)
    #print("layer = ", transformer_layers, layer_idx, ff_attrs)
